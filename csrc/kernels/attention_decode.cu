...
__global__ void flash_decode_splitk_kernel(
    ...
    constexpr int VEC2_PER_LANE = ELEMS / 2;
    ...
    const int context_len = cache_len;
    ...
    float q_reg[ELEMS];
    {
        ...
        for (int i = 0; i < VEC2_PER_LANE; i++) {
            const __nv_bfloat162 pair = reinterpret_cast<const __nv_bfloat162*>(q_ptr)[i];
            const float2 pair_f = __bfloat1622float2(pair);
            q_reg[2 * i]     = pair_f.x;
            q_reg[2 * i + 1] = pair_f.y;
        }
    }
    ...
    // -------------------------------------------------------------
    // 4) Main loop – iterate over the KV blocks that belong to this split
    // -------------------------------------------------------------
    for (int blk = split_start + warp_id; blk < split_end; blk += NUM_WARPS) {
        const long block_base =
            (((long)seq_idx * H_kv + h_kv) * max_seq_len + (long)blk * BLOCK_SIZE) * HEAD_DIM;
        ...
        for (int tok = 0; tok < tokens_in_block; tok++) {
            ...
            #pragma unroll
            for (int i = 0; i < VEC2_PER_LANE; i++) {
                const __nv_bfloat162 k_pair = reinterpret_cast<const __nv_bfloat162*>(k_ptr)[i];
                const __nv_bfloat162 v_pair = reinterpret_cast<const __nv_bfloat162*>(v_ptr)[i];
                const float2 k_f = __bfloat1622float2(k_pair);
                const float2 v_f = __bfloat1622float2(v_pair);
                partial += q_reg[2 * i] * k_f.x + q_reg[2 * i + 1] * k_f.y;
                v_reg[2 * i]     = v_f.x;
                v_reg[2 * i + 1] = v_f.y;
            }
            ...
            for (int e = 0; e < ELEMS; e++) {
                warp_acc[e] = warp_acc[e] * alpha + p * v_reg[e];
            }
        }
    }
...
extern "C" void launch_attention_decode_v4(
    ...
    const void* k_cache,         // [batch, n_kv_heads, max_seq_len, head_dim] (bfloat16, head‑major)
    ...
    int n_kv_heads,
    int head_dim,
    int max_seq_len,             // 与 k_cache/v_cache 第三维相同（用于 KV stride）
    float scale,
    cudaStream_t stream
) {
    // ------------------------------------------------------------------
